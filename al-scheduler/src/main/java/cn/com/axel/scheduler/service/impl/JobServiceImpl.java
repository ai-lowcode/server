...
import cn.com.axel.scheduler.execute.GeneralJobExecute;
import cn.com.axel.scheduler.mapper.JobMapper;
import cn.com.axel.scheduler.service.JobService;
...
public class JobServiceImpl extends ServiceImpl<JobMapper, Job> implements JobService {
    ...
    public JobServiceImpl(@Autowired @Qualifier("alSchedulerFactoryBean") SchedulerFactoryBean alSchedulerFactoryBean) {
...
//    @PostConstruct
    public void initJob() throws SchedulerException {
        Scheduler scheduler = alSchedulerFactoryBean.getScheduler();
        scheduler.clear();
    ...
    public Result<Job> insertTrigger(Job job) throws SchedulerException {
        ...
        //任务状态为启用时，创建调度策略
        if (result.isSuccess()) {
            SchedulerUtils.createScheduler(alSchedulerFactoryBean.getScheduler(), job, list, schedulerProperties.isCover());
    ...
    private void deleteTrigger(Job job) throws SchedulerException {
        ...
        if (subscribes != null && !subscribes.isEmpty()) {
            ...
            SchedulerUtils.removeTrigger(alSchedulerFactoryBean.getScheduler(), job, subscribes);
        }
    }
    ...
    public Result<Boolean> setStatus(Job job) throws SchedulerException {
        if (baseMapper.updateById(new Job().setId(job.getId()).setStatus(job.getStatus())) == 1) {
            ...
            if (job.getStatus() == 1) {
                SchedulerUtils.pause(alSchedulerFactoryBean.getScheduler(), newJob, subscribes);
            } else {
                SchedulerUtils.resume(alSchedulerFactoryBean.getScheduler(), newJob, subscribes);
            }
