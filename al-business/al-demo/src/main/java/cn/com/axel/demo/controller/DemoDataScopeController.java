...
import cn.com.axel.common.oauth.common.DataScopeType;
import cn.com.axel.demo.entity.DemoDataScope;
import cn.com.axel.demo.req.ReqDemoDataScope;
import cn.com.axel.demo.service.DemoDataScopeService;
...
public class DemoDataScopeController {
    ...
    @Operation(summary = "查询组织编码为admin或axel及其下级组织数据", description = "查询组织编码为admin或axel及其下级组织数据")
    @GetMapping("/adminOrg")
    @RequiresPermissions("demo:demoDataScope:query")
    @DataScope(table = "demo_data_scope", type = DataScopeType.Org, values = {"admin", "axel"})
    public Result<PageResult<DemoDataScope>> adminOrg(ReqDemoDataScope reqDemoDataScope, ReqPage reqPage) {
        return Result.ok(new PageResult<>(queryList(reqDemoDataScope, reqPage)), "查询组织编码为admin或axel及其下级组织数据成功!");
    }
